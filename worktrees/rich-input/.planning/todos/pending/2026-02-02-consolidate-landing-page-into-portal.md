...
title: Consolidate deleted app into portal (5173)
area: architecture
files:
  - deleted app source
  - deleted app source
...
2. **Client deleted app** (deleted app, port 5174) reads/writes to **JSON file storage** via Next.js API routes
...
**Goal:** `deleted app` should ONLY serve:
...
- All API routes currently in `deleted app source`
- Client portal pages
