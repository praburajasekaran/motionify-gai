...
<task type="auto">
  ...
  <action>
    ...
    2. Add '💬' or '💭' to NOTIFICATION_ICONS for 'comment_created'
...
<task type="auto">
  <name>Update deleted app CommentThread</name>
  <files>deleted app source
  <action>
    Apply same notification integration to deleted app source
  ...
  <verify>
    TypeScript compilation passes in deleted app
  </verify>
  <done>
...
<success_criteria>
- Client posts comment → Superadmin receives email (not client)
- Superadmin posts comment → Client receives email (not superadmin)
