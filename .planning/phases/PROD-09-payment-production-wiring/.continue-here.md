...
The Razorpay webhook was incorrectly located in the Next.js landing page (`deleted app source`). Since payments happen from the portal at port 5173 (which uses Netlify Functions as backend), the webhook was moved to `netlify/functions/razorpay-webhook.ts`.
