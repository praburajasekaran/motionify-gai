...
<task type="auto">
  ...
  <action>
    Create `deleted app source` by adapting `contexts/NotificationContext.tsx`:
  ...
  <verify>
    `npm run build` passes in deleted app directory
    File exports NotificationProvider and useNotifications
  </verify>
  <done>
    NotificationContext.tsx exists in deleted app source with all required exports
  </done>
</task>
...
<verification>
1. NotificationContext.tsx exists at `deleted app source`
