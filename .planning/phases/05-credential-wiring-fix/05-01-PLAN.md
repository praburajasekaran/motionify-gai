...
Add `credentials: 'include'` after the headers object, before the body.
  ...
  <verify>
Run: `cd deleted app && npm run build`
...
1. **Grep for credentials pattern in modified files:**
   ```bash
   grep -n "credentials: 'include'" deleted app source
...
2. **Both builds pass:**
   - Client portal: `cd deleted app && npm run build` (Next.js)
   - Admin portal: `npm run build` (Vite)

3. **No TypeScript errors:**
   - `cd deleted app && npx tsc --noEmit`
   - `npx tsc --noEmit`
</verification>
